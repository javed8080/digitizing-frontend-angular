...
export class NavService {
	...
	MENUITEMS: Menu[] = [
		...
		{
			title: 'Quotes', icon: 'airplay', type: 'sub', active: false, children: [
				{ path: '/customer/quotes/list', title: 'List', type: 'link' },
				{ path: '/customer/quotes/add', title: 'Add', type: 'link' }
			]
		},

		{
			title: 'Orders', icon: 'airplay', type: 'sub', active: false, children: [
				{ path: '/customer/orders', title: 'List', type: 'link' },
				{ path: '/customer/orders/add', title: 'Add', type: 'link' }
			]
		},

		{
			path: '/customer/orders/pending-list', title: 'Pending Orders', icon: 'box', type: 'link'
		},

		{
			path: '/customer/invoices', title: 'Invoices', icon: 'dollar-sign', type: 'link'
		},
	...
	// Filtered menu items based on URL parameters
	private filterMenuItems() {
		const path = window.location.pathname;
		console.log(path, 'path');
		if (path.includes('customer')) {
		  this.items.next(this.MENUITEMS.filter(menu => 
			menu.path?.includes('customer') || 
			menu.children?.some(child => child.path?.includes('customer'))
		  ));
		} else {
